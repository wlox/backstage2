  ...
  if (!no_reset) {
    if (intervals) {
      ...
      if (grid_totals) {
        ...
        gridTotals = new Array();
      }
      passiveValues = new Array();
    }
    $.ajax({
      url: url,
      beforeSend: function() {
      ...
      success: function(html) {
        ...
        if (custom_animation)
          ajaxAnimate(elemId, html, custom_animation);
        else {
          if (elem_id == 'edit_box' || elem_id == 'message_box' || elem_id == 'attributes box')
      ...
      data: ((get_elem) ? {
        bypass: 1,
        target_elem: elem_id,
        field_name: $("#" + get_elem).attr('name'),
        field_value: $("#" + get_elem).attr('value')
      } : {
        bypass: ((elem_id != 'edit_page' && !no_bypass) ? 1 : 0),
        target_elem: elem_id
      })
